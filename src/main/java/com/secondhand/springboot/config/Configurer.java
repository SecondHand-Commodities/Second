...
public class Configurer implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registration.excludePathPatterns("/","/login","/error","/static/**","/logout","/login_check","/signup","/register");// 默认不拦截路径 需要就改
        
        //添加权限拦截器
        InterceptorRegistration registration1 = registry.addInterceptor(new LimitInteceptor());
       registration1.addPathPatterns("/login");
        registration1.excludePathPatterns("/","/login","/error","/static/**","/logout","/signup","/register");// 默认不拦截路径 需要就改
    }
    @Override
